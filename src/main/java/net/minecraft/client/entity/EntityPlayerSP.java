...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
        EventMotion event = new EventMotion(this.posX, this.getEntityBoundingBox().minY, this.posZ,this.rotationYaw, this.rotationPitch, this.onGround,true);
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            val rotationYaw = event.getYaw();
            val rotationPitch = event.getPitch();
            val posX = event.getX();
            val posY = event.getY();
            val posZ = event.getZ();
            ...
            if (this.isRiding())
            {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(posX, -999.0D, posZ, rotationYaw, rotationPitch, onGround));
                flag2 = false;
            }
            else if (flag2 && flag3)
            {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(posX, posY, posZ, rotationYaw, rotationPitch, onGround));
            }
            else if (flag2)
            {
                this.connection.sendPacket(new CPacketPlayer.Position(posX, posY, posZ, onGround));
            }
            else if (flag3)
            {
                this.connection.sendPacket(new CPacketPlayer.Rotation(rotationYaw, rotationPitch, onGround));
            }
        ...
        EventMotion post = new EventMotion(this.posX, this.getEntityBoundingBox().minY, this.posZ,this.rotationYaw, this.rotationPitch, this.onGround,false);
        Client.getInstance().getEventManager().call(post);
    ...
    public void sendChatMessage(String message) {
        ...
        if (eventChat.isCancelled()) return;
