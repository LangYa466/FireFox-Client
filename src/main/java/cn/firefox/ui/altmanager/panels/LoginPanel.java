...
import cn.firefox.ui.altmanager.ActionButton;
...
import cn.firefox.ui.altmanager.text.RandomStringGenerator;
...
public class LoginPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        for (ActionButton actionButton : actionButtons) {
            ...
            actionButton.setClickAction(() -> {
                switch (actionButton.getName()) {
                    case "Login":
                        Client.getInstance().getAltManager().getUtils().login(textFields.get(0), textFields.get(1));
                        resetTextFields();
                    ...
                    case "Add":
                        ...
                        break;
                    case "Gen Cracked":
                        Client.getInstance().getAltManager().getUtils().loginWithString(generator.generate(8), "", false);
                        Client.getInstance().getAltManager().getAltPanel().refreshAlts();
                        break;
                }
            });
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("firefox/mc.png"));
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (hoveringMicrosoft && button == 0) {
            ...
            Client.getInstance().getAltManager().getUtils().microsoftLoginAsync(email, password);
            resetTextFields();
        }
