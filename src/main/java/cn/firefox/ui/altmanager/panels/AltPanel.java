package cn.firefox.ui.altmanager.panels;

import cn.firefox.Client;
import cn.firefox.manager.notification.NotificationType;
import cn.firefox.ui.altmanager.DoubleIconButton;
import cn.firefox.ui.altmanager.TextField;
import cn.firefox.ui.altmanager.misc.*;
import cn.firefox.ui.altmanager.Scroll;
import cn.firefox.ui.altmanager.ban.HypixelBan;
...
import cn.firefox.util.render.StencilUtil;
...
public class AltPanel extends Panel {
    ...
    public void keyTyped(char typedChar, int keyCode) {
        if (Client.getInstance().getAltManager().isTyping()) return;
        ...
        if (GuiScreen.isKeyComboCtrlV(keyCode)) {
            String[] lines = IOUtil.getClipboardString().split("\n");
        ...
        if (GuiScreen.isKeyComboCtrlC(keyCode)) {
            ...
            Client.getInstance().getNotificationManager().post(NotificationType.SUCCESS, "Copied " + count + " alts to clipboard");
            IOUtil.copy(stringBuilder.toString());
            return;
        }
        ...
        if (keyCode == Keyboard.KEY_DELETE) {
            ...
            refreshAlts();
            Client.getInstance().getNotificationManager().post(NotificationType.SUCCESS, "Removed " + count + " alts");
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        TextField searchField = Client.getInstance().getAltManager().searchField;
        boolean filterBanned = Client.getInstance().getAltManager().filterBanned.isEnabled();
        ...
        for (AltRect altRect : visibleAltRects) {
            ...
            altRect.setX(altX);
            altRect.setY((float) (altY + MathUtil.roundToHalf(scroll.getScroll())));
    ...
    public static class AltRect implements Screen {
        ...
        private final DoubleIconButton favoriteButton = new DoubleIconButton(IconUtil.STAR_OUTLINE, IconUtil.STAR);
        ...
        public void drawScreen(int mouseX, int mouseY) {
            ...
            switch (alt.getType()) {
                ...
                case "Mojang":
                    ...
                    mc.getTextureManager().bindTexture(new ResourceLocation("firefox/mojang.png"));
                    Gui.drawModalRectWithCustomSizedTexture(typeX + 2, usernameY - 1, 0, 0, size, size, size, size);
                    RenderUtil.resetColor();
                    break;
                case "Cracked":
                    tenacityBoldFont14.drawString(" Cracked", typeX, usernameY + 1, ColorUtil.applyOpacity(Color.red, 1 - credsAnim));
                    break;
                case "Not logged in":
            ...
            if (Client.getInstance().getAltManager().currentSessionAlt == alt) {
                tenacityBoldFont14.drawString("Logged in", x + width - (tenacityBoldFont14.getStringWidth("Logged in") + 5),
                        y + height - (tenacityBoldFont14.getHeight() + 5), Color.green);
            } else {
                if (loadingAltRect == this) {
                    float iconWidth = iconFont16.getStringWidth(IconUtil.REFRESH);
                    ...
                    iconFont16.drawString(IconUtil.REFRESH, iconX, iconY, ColorUtil.applyOpacity(-1, .5f));
        ...
        public void drawAltHead(float x, float y, float size) {
            Client.getInstance().getAltManager().getUtils().getHead(alt);
            GLUtil.startBlend();
            RenderUtil.setAlphaLimit(0);
            mc.getTextureManager().bindTexture(alt.head != null ? alt.head : new ResourceLocation("firefox/x-steve.png"));
